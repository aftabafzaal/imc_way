...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
    ...
    <div class="kt-subheader__toolbar">
		...
		<a href="{{ url('admin/benefit/create') }}" class="btn btn-label-brand btn-bold">Create Benefit </a>
		<a href="{{ url('admin/benefit/section') }}" class="btn btn-label-brand btn-bold">Benefit Sections</a>          
    </div>
</div>
         ...
         <div class="kt-datatable kt-datatable--default kt-datatable--brand kt-datatable--loaded" id="kt_apps_user_list_datatable" style="">
					  ...
					  <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
					    <td class="kt-datatable__cell--center kt-datatable__cell kt-datatable__cell--check" data-field="id">
                          <span style="width: 20px;"><label class="kt-checkbox kt-checkbox--single kt-checkbox--solid">
                              <input type="checkbox" name="id[]" class="childBox" value="{{$benefit->id}}">&nbsp;<span></span></label>
                            </span>
                        </td>
						<td class="kt-datatable__cell--sorted kt-datatable__cell" data-field="title_en">
							<span style="width: 200px;">
							    <span style="width: 206px;">{{$benefit->title_en}}</span>
							</span>
                        </td>
                        <td class="kt-datatable__cell--sorted kt-datatable__cell" data-field="title_en">
                            <span style="width: 200px;">
                                <span style="width: 206px;">{{$benefit->title_ar}}</span>
                            </span>
                        </td>
						 <td class="kt-datatable__cell"><span style="width: 206px;">{{date('d-m-Y H:i:s',strtotime($benefit->created_at))}}</span></td>
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">									   
										<li class="kt-nav__item"><a class="kt-nav__link" href="{!!url('admin/benefit')!!}/{!!$benefit->id!!}/edit"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
									 	<li onclick="deleteThis(this, {{$benefit->id}})" class="kt-nav__item delete_user"><a class="kt-nav__link delete_user" href="javascript:void(0);"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text delete_user">Delete</span>
									 </ul>
								  </div>
							   </div>
							</span>
            ...
            <div class="kt-datatable__pager kt-datatable--paging-loaded">
                ...
                <div class="kt-datatable__pager-info">
                    ...
                    <span class="kt-datatable__pager-detail">Showing {{$benefits->firstItem()}} - {{$benefits->lastItem()}} of {{$benefits->total()}}</span>
                </div>
			</div>
         </div>
         <!--end: Datatable -->
...
<script>
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			text: "Are you sure to delete this benefit?",
			type: "danger",
	...
	$(document).ready(function() {
		...
		$('#kt_subheader_group_actions_delete_all').on('click', function() {
			...
			if (ids.length > 0) {
				...
				swal.fire({
					buttonsStyling: false,
					text: "Are you sure to delete " + ids.length + " selected benefits?",
					type: "danger",
					confirmButtonText: "Yes, delete!",
					confirmButtonClass: "btn btn-sm btn-bold btn-danger",
					showCancelButton: true,
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "{{url('admin/benefit/deleteMultiple')}}",
							type: 'POST',
