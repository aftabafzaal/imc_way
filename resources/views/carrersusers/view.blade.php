...
@section('title', 'Submitted Information by users')
...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        <div class="kt-subheader__main">
            <h3 class="kt-subheader__title">
                Submitted Information by users
            </h3>
            <span class="kt-subheader__separator kt-subheader__separator--v"></span>
            <div class="kt-subheader__group" id="kt_subheader_search">
                <span class="kt-subheader__desc" id="kt_subheader_total">
				{{$users->total()}} Total</span>
                <form class="kt-margin-l-20" id="kt_subheader_search_form" action="{{url('/carrersusers/listing')}}" method="GET" role="search">
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('carrersusers/listing?key=user&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $users->currentPage()}}">
							Name
				...
				<th data-field="Country" class="kt-datatable__cell kt-datatable__cell--sort">
				   <span style="width: 206px;">
						<a href="{{url('carrersusers/listing?key=email&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $users->currentPage()}}">
							Email
     ...
     <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <h6><b>JOB : {{$user->job}}</b></h6>
            <h6><b>Residency : {{$user->residency}}</b></h6>
            <h6><b>Natinality : {{$user->nationality}}</b></h6>
            <h6><b>Number : {{$user->number}}</b></h6>
            <h6><b>Ip : {{$user->ip}}</b></h6>

            <?php
            $helper = new App\Helpers();
            ?>
            <?php
            if(!empty($user->media_resume)){
            $enImage= $helper->getImage($user->media_resume);
            $img=env('BASE_URL').$enImage;
            ?>
            <h6><b>Resume : <a href="<?php echo $img;?> ">View Resume</a></b></h6>
            <?php
             }
            ?>
          </div>
	     ...
	     <tr data-row="0" class="kt-datatable__row" style="left: 0px;">
             ...
             <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;"><button class="btn-success" data-toggle="modal" data-target="#myModal{{$user->id}}">View</button></span></td>
						 ...
						 <td data-field="Actions" data-autohide-disabled="false" class="kt-datatable__cell">
							<span style="overflow: visible; position: relative; width: 80px;">
                                 ...
                                 <?php
                                  $helper = new App\Helpers();
                                  $user=Auth::user();
                                  $checkpermision= $helper->checkpermission($user,1,"delete");
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('carrersusers/delete-single-user'); ?>",
					type: 'POST',
	...
	function blockThis(obj, id, status)
	{
		...
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('carrersusers/change-status'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('carrersusers'); ?>" + '/delete-multiple-users',
							type: 'POST',
	...
	if (ids.length > 0) {
		...
		swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('carrersusers'); ?>" + '/update-multi-users-status',
							type: 'POST',
