...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('events/create') }}" class="btn btn-label-brand btn-bold">
                Add Events  </a>
                <a href="{{ url('events/banner') }}" class="btn btn-label-brand btn-bold">
                          Event Banners  </a>
        </div>
    </div>
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('events/listing?key=name&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Title
				...
				<th data-field="Country" class="kt-datatable__cell kt-datatable__cell--sort">
				   <span style="width: 206px;">
						<a href="{{url('events/listing?key=description&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Description
				...
				<th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 206px;">
						<a href="{{url('events/listing?key=date&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ).'&page=' . $pages->currentPage()}}">
						Date created
        ...
        <th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
          ...
          Image
           ...
           ?>
             ...
             <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;"><img width="32" height="32" src="<?php echo $img;?>"></img></span></td>
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">
										<li class="kt-nav__item"><a class="kt-nav__link" href="{{url('events/edit', $page->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('events/delete-single-page'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('events'); ?>" + '/delete-multiple-pages',
							type: 'POST',
