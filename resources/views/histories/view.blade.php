...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('histories/create') }}" class="btn btn-label-brand btn-bold">
                Add History </a>

                <a href="{{ url('histories/title') }}" class="btn btn-label-brand btn-bold">
                Add Section </a>
        </div>
			   ...
			   <tr class="kt-datatable__row" style="left: 0px;">
				...
				<th data-field="AgentName" class="kt-datatable__cell kt-datatable__cell--sort ">
					<span style="width: 200px;">
						<a href="{{url('histories/listing?key=name&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ). '&page=' . $pages->currentPage()}}">
							Title
				...
				<th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
					<span style="width: 206px;">
						<a href="{{url('histories/listing?key=date&sort_by=').session()->get( 'sort_by' ).'&recordvalue=' . session()->get( 'recordvalue' ).'&page=' . $pages->currentPage()}}">
						Date created
							...
							<span style="overflow: visible; position: relative; width: 80px;">
							   <div class="dropdown">
								  ...
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">
										<li class="kt-nav__item"><a class="kt-nav__link" href="{{url('histories/edit', $page->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li>
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('histories/delete-single-page'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('histories'); ?>" + '/delete-multiple-pages',
							type: 'POST',
