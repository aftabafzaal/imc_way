...
<section class="banner-event-inner">
	...
	<div class="owl-carousel owl-carousel-event owl-theme">
		...
		@php
			$enImage= $helper->getImage($item->media_id);
			$enattr= $helper->getimageattirbute($enImage);

	   @endphp
			<div class="item">
				<a href="{{!empty($item->link)?$item->link:"#"}}">
					<img src="{{env('BASE_URL')}}<?php echo $enImage?>"  alt="{{ __('messages.eventbannerAlternate') }}" title="{{ __('messages.eventbannerTitle') }}">
				</a>
			</div>
		@endforeach
	</div>
		...
		<div class="row allevents">
		   ...
		   @endphp
			<div class="col-lg-4 col-md-6 event" style="margin-bottom: 40px;">
				<a href="{{Config::get('variable.URL_LINK')}}event/{{($language == "en")?$event->slug_en:$event->slug_ar}}" class="comingevent-box">
...
<script type="text/javascript">
	...
	$(".loadmore").click(function(){
		...
		$('.pageloader').show();
		$.ajax({
			url: "{{Config::get('variable.URL_LINK')}}event/loadMore",
			type: 'post',
