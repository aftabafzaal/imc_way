...
<div class="kt-subheader   kt-grid__item" id="kt_subheader">
        ...
        <div class="kt-subheader__toolbar">
            ...
            <a href="{{ url('testimonials/create') }}" class="btn btn-label-brand btn-bold">
                Add Testimonial </a>
        </div>
    </div>
        ...
        <th data-field="ShipDate" class="kt-datatable__cell kt-datatable__cell--sort">
          ...
          Video
           ...
           ?>
             ...
             <td data-field="ShipDate" class="kt-datatable__cell"><span style="width: 206px;">

             	@if(!empty($img) and empty($page->youtube_url))
               <video width="32" height="32" controls>
                <source src="<?php echo $img;?>" type="video/mp4">
              </video>
              @elseif(!empty($page->youtube_url))
						 ...
						 <td data-field="Actions" data-autohide-disabled="false" class="kt-datatable__cell">
							<span style="overflow: visible; position: relative; width: 80px;">
							     <?php
                                  $helper = new App\Helpers();
                                  $user=Auth::user();
                                  $checkpermision1= $helper->checkpermission($user,3,"edit");
                                   $checkpermision2= $helper->checkpermission($user,3,"delete");
                                   if($checkpermision1 == true || $checkpermision2 == true){
                                   ?>
							   <div class="dropdown">
								  <a data-toggle="dropdown" class="btn btn-sm btn-clean btn-icon btn-icon-md"><i class="flaticon-more-1"></i></a>
								  <div class="dropdown-menu dropdown-menu-right">
									 <ul class="kt-nav">
									 <?php if($checkpermision1 == true){?><li class="kt-nav__item"><a class="kt-nav__link" href="{{url('testimonials/edit', $page->id)}}"><i class="kt-nav__link-icon flaticon2-contract"></i><span class="kt-nav__link-text">Edit</span></a></li><?php }?>
									 <?php  if($checkpermision2 == true){?>	<li onclick="deleteThis(this, {{$page->id}})" class="kt-nav__item delete_user"><a class="kt-nav__link delete_user" href="javascript:void(0);"><i class="kt-nav__link-icon flaticon2-trash"></i><span class="kt-nav__link-text delete_user">Delete</span><?php }?>
									 </ul>
								  </div>
							   </div><?php 
}
	...
	function deleteThis(obj, id)
	{
		swal.fire({
			...
			if (result.value) {
				$('.pageloader').show();
				$.ajax({
					url: "<?php echo url('testimonials/delete-single-page'); ?>",
					type: 'POST',
			...
			if (ids.length > 0) {
				...
				swal.fire({
					...
					if (result.value) {
						$('.pageloader').show();
						$.ajax({
							url: "<?php echo url('testimonials'); ?>" + '/delete-multiple-pages',
							type: 'POST',
