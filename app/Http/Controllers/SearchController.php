      ...
      $mainsectionFooter1 = Section::where('id',4)->where('is_enable','1')->first();
       if(!empty($mainsectionFooter1)){
        ...
        if(isset($request->searchText) && !empty($request->searchText)){
            $search_data = $this->getSearchResult($request->searchText,$request->lng);
        }   

        return view ('webpages.search', compact('followusSection','topmenu_0','topmenu_1','topmenu_2','topmenu_3','topmenu_4','middlemenu_1','middlemenu_2','middlemenu_3','middlemenu_4','middlemenu_5','Mainmenu_1','Mainmenu_2','Mainmenu_3','Mainmenu_4','Mainmenu_5','Mainmenu_6','footermenu_1','footermenu_2','footermenu_3','footerContact','Bootomfooter','Know_Imc','News','testimonies','Award','Affiliates','slider','footermenu_1_sub','footermenu_2_sub','footermenu_3_sub','Mainmenu_1_sub','Mainmenu_2_sub','Mainmenu_3_sub','Mainmenu_4_sub','Mainmenu_5_sub','Mainmenu_6_sub','section','topmenu_section','middlemenu_section','Mainmenu_section','footermenu_section','Settings','search_data'));
    }

    public function getSearchResult($searchText,$lng){

        $tables = array("academies","appointments","awards","benefits","events","leadership_messages","patientrights","residences","staying_imcs","surveys","visitors");
        ...
        $urls = array(
          ...
          "patientrights" => "patientright",
          "residences" => "residence",
          "staying_imcs" => "stayingimc",
          "surveys" => "survey",
          "visitors" => "visitor"
        );
        ...
        foreach($tables as $table){
          if($lng == "ar"){
            $sql = "SELECT title_en,title_ar,description_en,description_ar FROM ".$table." WHERE description_ar like '%". $searchText."%'";
          }else{
            $sql = "SELECT title_en,title_ar,description_en,description_ar FROM ".$table." WHERE description_en like '%". $searchText."%'";
          }
          $search_result = DB::select($sql);
          if(count($search_result)){
            $search_data[$urls[$table]] = $search_result;
          }
        }
        return $search_data;
    }    
