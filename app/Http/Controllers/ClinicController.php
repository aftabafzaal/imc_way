...
class ClinicController extends Controller
{
    ...
    if(!empty($mainsection)){
      ...
      $mainsectionFooter1 = Section::where('id',4)->where('is_enable','1')->first();
       if(!empty($mainsectionFooter1)){
        ...
        $clinics = $this->getSearchClinic($request);
        $searchLanguage = isset($request->language)?$request->language:"arabic";
        ...
        if(isset($request->clinicname)){
            $queryString .= empty($queryString)?"clinicname=$request->clinicname":"&clinicname=$request->clinicname";
        }
        if(isset($request->gender)){
            $queryString .= empty($queryString)?"gender=$request->gender":"&gender=$request->gender";
        }

        return view ('webpages.clinics', compact('followusSection','topmenu_0','topmenu_1','topmenu_2','topmenu_3','topmenu_4','middlemenu_1','middlemenu_2','middlemenu_3','middlemenu_4','middlemenu_5','Mainmenu_1','Mainmenu_2','Mainmenu_3','Mainmenu_4','Mainmenu_5','Mainmenu_6','footermenu_1','footermenu_2','footermenu_3','footerContact','Bootomfooter','Know_Imc','News','testimonies','Award','Affiliates','slider','footermenu_1_sub','footermenu_2_sub','footermenu_3_sub','Mainmenu_1_sub','Mainmenu_2_sub','Mainmenu_3_sub','Mainmenu_4_sub','Mainmenu_5_sub','Mainmenu_6_sub','section','topmenu_section','middlemenu_section','Mainmenu_section','footermenu_section','Settings','clinics','queryString','languages','searchLanguage'));
    }

    public function getSearchClinic($request){

        // $page = isset($request->page)?$request->page-1:0;
        // $clinicName = isset($request->clinicname)?$request->clinicname:"";
        // $language = isset($request->language)?$request->language:"arabic";
        // $searchData = array(
        //     "physicianId"   => "",
        //     "isActive"      => "Y",
        //     "search_txt"    => $clinicName,
        //     "item_count"    => "20",
        //     "page"          => $page,
        //     "lang"          => $language,
        // );
        ...
        // $ch = curl_init('http://192.168.1.49:8080/imc_portal/api/clinics/find/');
    ...
    public function infromationsave(Request $request)
    {
      ...
      $id=Symptom::insertGetId($data);
     return response()->json(['status' => true,'message'=>'Saved data successfully.','data'=>['id'=>$id]],200);
    }
    
    public function inforesultsave(Request $request){
        
           header('Access-Control-Allow-Origin: *');
           Symptom::where('id',$request['id'])->update(['result'=>$request['result']]);
           $records = (json_decode($request['result']));
           $code = [];
           foreach($records as $record){
            if(isset($record->id) && $record->id != ''){
                $detail = ClinicMapping::select('clinic_name','clinic_name_ar','clinic_code')->where('id',$record->id)->get(); 
                   array_push($code,$detail);
           }
         }
           //dd($code);
          
           
          return response()->json(['status' => true,'message'=>'Saved data successfully.','clinic_detail'=>$code,'data'=>[]],200); 
    }
      ...
      public function getItem(Request $request)
    {
      ...
      $getdata=Item::where('user_id',$request['user_id'])->orderBy("id",'desc')->first();
      if(!empty($getdata)){
           $item_count= $getdata->item;
           Item::where('id',$getdata->id)->delete();
      }else{
          $item_count=0;
      }
     return response()->json(['status' => true,'message'=>'Saved data successfully.','data'=>['item_count'=>$item_count]],200);
    }
