      ...
      $request['description_ar'] = trim($request['description_ar']);
        if(!isset($request->id)){
            $this->validate($request,[
                ...
                'image1.required' => 'Please upload the image',
                'image1Ar.required' => 'Please upload the image',
            ]);
        }else{
            $this->validate($request,[
                'title_en' => 'required|min:3|max:255',
        ...
        if(isset($request->id)) {
              ...
              if(!empty($request->image1Ar)) {
                if(!empty($request->image1Ar) && $request->image1Ar != null) {
                   $mediaid= $helper->getMediaID($request->image1Ar);
                   $array['image1Ar']= $request->image1Ar;
            ...
            if(!empty($request->pdf)) {
                  if($request->pdf != $existingdata->pdf){
                      $file = $helper->upload_image($request->pdf,'images/events/','none');
                      $array['media_pdf'] = $file;
                  }
            ...
            if(count($request->speakers)>0){
                    ...
                    foreach($request->speakers as $speaker){
                       if($speaker["name_en"] != null){
                           ...
                           if(!empty($speaker["image"]) && $speaker["image"] != null) {
                              $mediaid= $helper->getMediaID($speaker["image"]);
                             $newSpeaker->image = $speaker["image"];
                              $newSpeaker->media_id_en  = $mediaid;
                           }
                           if(!empty($speaker["imageAr"]) && $speaker["imageAr"] != null) {
                              $mediaid= $helper->getMediaID($speaker["imageAr"]);
                             $newSpeaker->image = $speaker["imageAr"];
                              $newSpeaker->media_id_ar  = $mediaid;
                           }
            ...
            if(count($request->banners)>0){
                ...
                foreach($request->banners as $banner){
                      ...
                      if(!empty($banner["image"]) && $banner["image"] != null) {
                         $mediaid= $helper->getMediaID($banner["image"]);
                        $newBanner->image = $banner["image"];
                         $newBanner->media_id_en  = $mediaid;
                      }
                      if(!empty($banner["imageAr"]) && $banner["imageAr"] != null) {
                         $mediaid= $helper->getMediaID($banner["imageAr"]);
                        $newBanner->imageAr = $banner["imageAr"];
                         $newBanner->media_id_ar  = $mediaid;
                      }
                      $newBanner->save();

                }
            }
//  $a=Eventbanner::where("event_id",$request->id)->get();
//  dd($a);
            ...
            if(count($request->speakers)>0){
                    foreach($request->speakers as $speaker){
                        ...
                        if(!empty($speaker["image"]) && $speaker["image"] != null) {
                             $mediaid= $helper->getMediaID($speaker["image"]);
                              $newSpeaker->imageAr = $speaker["image"];
                              $newSpeaker->media_id_en = $mediaid;
                        }
                        if(!empty($speaker["imageAr"]) && $speaker["imageAr"] != null) {
                             $mediaid= $helper->getMediaID($speaker["imageAr"]);
                              $newSpeaker->imageAr = $speaker["imageAr"];
                              $newSpeaker->media_id_ar = $mediaid;
                        }
                        $newSpeaker->save();
                    }
    ...
    public function listing(Request $request)
    {
          ...
          foreach($all as $key=>$v){
           //Event::where('id',$v->id)->update(['slug_en'=>str_slug($v['title_en'],'-'),'slug_ar'=>$v['title_ar'] ]);
            //Event::where('id',$v->id)->update(['slug_en'=>str_slug($v['title_en'],'-'),'slug_ar'=>str_slug($v['title_en'],'-') ]);
          }
        ...
        $pages = Event::select('id','title_en','image1','description_en','title_ar','description_ar','created_at','media_id_en','media_id_ar');
...
$page['eventbanner']=Eventbanner::where("event_id",$id)->get();
$page['eventspeakers']      =  Eventspeaker::where("event_id",$id)->get();
$page['eventcourses']      =       Eventcourse::where("event_id",$id)->get();
